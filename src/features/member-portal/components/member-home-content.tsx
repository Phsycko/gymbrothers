		...
		return (
			<div className="space-y-6">
				...
				<div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5 text-sm text-amber-100 backdrop-blur-xl">
					No encontramos una ficha de socio asociada a tu correo. Si ya eres miembro,
					pide en recepción que vinculen tu email o contacta al staff.
				</div>
			</div>
		);
	}
							...
							{status.isActive ? (
								<>
									...
									<p className="text-xs text-white/55">
										{status.daysRemaining === 1 ? "día restante" : "días restantes"}
									</p>
								</>
							) : (
					...
					<ul className="space-y-3">
						{preview.map((a) => (
							...
							>
								...
								<p className="mt-1 line-clamp-2 text-sm text-white/60">{a.content}</p>
							</li>
						))}
					</ul>
				)}
