...
export function DashboardHeader({
	...
	return (
		<header className="sticky top-0 z-30 flex h-14 min-w-0 items-center gap-2 border-b border-slate-800/80 bg-slate-950/70 px-3 pt-safe backdrop-blur-md sm:h-16 sm:gap-3 sm:px-4 md:gap-4 md:px-6">
			<Link
				href="/dashboard"
				className="hidden shrink-0 rounded-lg outline-none ring-offset-2 ring-offset-slate-950 focus-visible:ring-2 focus-visible:ring-[#E11D48]/60 md:block"
				aria-label="Inicio — GYM BROTHERS"
			>
				<AppLogo size="sm" className="md:h-9" />
			</Link>
			<div className="relative flex min-w-0 flex-1 items-center">
				<Search className="pointer-events-none absolute left-2.5 h-4 w-4 text-slate-500 sm:left-3" />
				<Input
					ref={searchRef}
					type="search"
					placeholder="Buscar…"
					className="h-9 min-w-0 border-slate-800 bg-slate-900/50 pl-9 pr-10 text-sm text-slate-100 placeholder:text-slate-500 sm:h-10 sm:pl-10 sm:pr-24 sm:text-base"
					aria-label="Búsqueda global"
				/>
				<kbd className="pointer-events-none absolute right-2 hidden h-6 select-none items-center gap-1 rounded border border-slate-700 bg-slate-800/80 px-1.5 font-mono text-[10px] font-medium text-slate-400 sm:right-3 md:flex">
